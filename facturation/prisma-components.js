...
// ============================================================================
// PrismaDocStore — Stockage des documents dans le dossier de l'application
// (File System Access API : Chrome/Edge. Le handle du dossier choisi par
// l'utilisateur est persisté en IndexedDB ; les fichiers sont écrits dans un
// sous-dossier `documents/` du dossier sélectionné.)
// ============================================================================

const PrismaDocStore = {
    DB_NAME: '_prisma_docstore',
    STORE_NAME: 'handles',
    HANDLE_KEY: 'rootDirectory',
    ROOT_DIR: 'documents', // sous-dossier créé dans le dossier choisi par l'utilisateur
    ...
    async _idbRequest(mode, fn) {
        const db = await this._openDB();
        try {
            return await new Promise((resolve, reject) => {
                const tx = db.transaction(this.STORE_NAME, mode);
                const req = fn(tx.objectStore(this.STORE_NAME));
                req.onsuccess = () => resolve(req.result);
                req.onerror = () => reject(req.error);
            });
        } finally {
    ...
    async loadHandle() {
        if (this._handleLoaded) return this.rootHandle;
        this._handleLoaded = true;
        try {
            this.rootHandle = (await this._idbRequest('readonly', s => s.get(this.HANDLE_KEY))) || null;
        } catch (e) {
            console.warn('[PrismaDocStore] Lecture du handle impossible:', e);
    ...
    async getStatus() {
        ...
        try {
            ...
            return perm === 'granted' ? 'connected' : 'need-permission';
        } catch (e) {
    ...
    /**
     * Redemande la permission sur le handle persisté.
    ...
    async requestAccess() {
        ...
        try {
            ...
            console.warn('[PrismaDocStore] requestPermission:', e);
            return false;
        }
    },

    /**
     * Ouvre le sélecteur de dossier et persiste le handle.
     * Doit être appelé depuis un geste utilisateur (clic).
     */
    async connect() {
        if (!this.isSupported()) {
            throw new Error('Navigateur non compatible (utiliser Chrome ou Edge).');
        }
        const handle = await window.showDirectoryPicker({ mode: 'readwrite' });
        this.rootHandle = handle;
        this._handleLoaded = true;
        try {
            await this._idbRequest('readwrite', s => s.put(handle, this.HANDLE_KEY));
        } catch (e) {
            console.warn('[PrismaDocStore] Persistance du handle impossible:', e);
        }
        return handle;
    },

    /** Oublie le dossier connecté (les fichiers restent sur le disque). */
    async disconnect() {
        this.rootHandle = null;
        this._handleLoaded = true;
        try {
            await this._idbRequest('readwrite', s => s.delete(this.HANDLE_KEY));
    ...
    /** Nettoie un nom de dossier (caractères interdits Windows). */
    sanitizeSegment(name) {
        const clean = String(name || '')
            ...
            .replace(/[. ]+$/g, '')
            .trim();
        return (clean || '_').slice(0, 80);
    },

    /** Nettoie un nom de fichier en préservant l'extension. */
    sanitizeFileName(name) {
        const raw = String(name || 'document');
        const dot = raw.lastIndexOf('.');
        const base = dot > 0 ? raw.slice(0, dot) : raw;
        const ext = dot > 0 ? raw.slice(dot + 1) : '';
        const cleanBase = this.sanitizeSegment(base).slice(0, 60) || 'document';
        const cleanExt = ext ? '.' + this.sanitizeSegment(ext).slice(0, 10) : '';
        return cleanBase + cleanExt;
    },

    async _getDir(segments, create) {
        const root = await this.loadHandle();
        if (!root) throw new Error('Aucun dossier de documents connecté.');
        let dir = await root.getDirectoryHandle(this.ROOT_DIR, { create });
        for (const seg of (segments || [])) {
            dir = await dir.getDirectoryHandle(this.sanitizeSegment(seg), { create });
        }
        return dir;
    },

    /**
     * Écrit un fichier dans documents/<segments...>/<fileName>.
     * @returns {{segments: string[], fileName: string}} chemin réellement utilisé (après nettoyage)
     */
    async saveFile(segments, fileName, fileOrBlob) {
        const dir = await this._getDir(segments, true);
        ...
        return {
            segments: (segments || []).map(s => this.sanitizeSegment(s)),
            fileName: safeName
        };
    },

    /** Lit un fichier → File (lève NotFoundError s'il a été déplacé/supprimé). */
    async readFile(segments, fileName) {
        const dir = await this._getDir(segments, false);
        const fh = await dir.getFileHandle(fileName);
        return await fh.getFile();
    },

    async deleteFile(segments, fileName) {
        const dir = await this._getDir(segments, false);
        await dir.removeEntry(fileName);
    },

    /** Convertit une data URL (base64) en Blob — pour migrer les anciennes pièces. */
