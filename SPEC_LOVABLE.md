...
| Persistance | `localStorage` (8 clés JSON) | Supabase Postgres (8 tables) ou KV équivalent |
...
**Convention obligatoire** : chaque table doit posséder un champ `client_data jsonb` (snapshot du client au moment de l'émission) pour les modules factures/devis/recus/propositions. **Ne jamais lire le client en live au rendu** : un document doit conserver l'état du client tel qu'il était à l'émission.
...
### 4.2 Liste des CDI (Centres de rattachement)

Liste à fournir dans le `<Select>` avec **optgroups** :

- **Yaoundé - CDI** : CDI YDE 1, CDI YDE 2, CDI YDE 3, CDI YDE 4, CDI YDE 5, CDI YDE 6
- **Yaoundé - CIME** : CIME YDE EST, CIME YDE OUEST
- **Yaoundé - Autres** : CSIPLI MFOUNDI
- **Autres régions** : CDI MEFOU ET AFAMBA, CDI Douala, CDI Bafoussam, CDI Garoua
- **Autre** (saisie libre)
...
- **Immo** : badges PSL (bleu), Bail (vert avec taux), TF (orange), `-` si aucun
- **Fiscal** : badges
...
|---|---|---|---|
| + PSL | bleu | Ajoute ligne `Précompte sur Loyer (PSL)` au montant `client.psl` | client.psl > 0 |
| + Bail | teal | Ajoute ligne `Bail Commercial` au montant `client.bail` | client.bail > 0 |
| + TF | orange | Ajoute ligne `Taxe Foncière (TF)` au montant `client.tf` | client.tf > 0 |
...
- Mémorise `importedDevisId` et `importedDevisNumber` → après émission, le devis passe à `status: 'converti'` avec `convertedToFacture: factureNumber`.
...
- **Imprimer** : `window.print()` (le rendu est déjà encapsulé dans `.print-area`)
- **PDF** : `html2canvas(printArea, { scale: 2 })` → split en pages A4 (210 × 297 mm, marges 10 mm) → jsPDF
...
| Champ supplémentaire rendu | "Bon pour accord" + date/signature client | (aucun) |
...
1. Créer une facture qui pré-importe ce devis
2. Rediriger vers le formulaire facture déjà rempli
3. À la sauvegarde de la facture, marquer `devis.status = 'converti'` et `devis.convertedToFacture = facture.number`
...
### 6.6 Toutes les autres règles (lignes de prestations, listes prédéfinies, boutons rapides, mode auto/manuel) sont **strictement identiques** à celles des factures (cf. §5.5 à §5.10).
...
- **Conversion montant en lettres** (français) : implémenter `numberToWords(num)` ; couvre 0 à 999 999 999. Voir `recu-app.html` ligne 499 pour l'algorithme exact.
...
- [ ] Les calculs IGS / Patente / TDL / PSL / Bail / TF / Solde IR donnent **la même valeur au franc près**
...
- [ ] Le changement d'un client (signature/cachet) **ne modifie pas** les documents déjà émis (snapshot `clientData`)
