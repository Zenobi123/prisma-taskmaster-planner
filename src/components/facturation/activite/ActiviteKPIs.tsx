...
const ActiviteKPIs = ({ data }: Props) => {
  const kpis: KPICardProps[] = [
    ...
    {
      label: "Impôts & taxes",
      value: `${fmt(data.totalImpots)} F CFA`,
      icon: Receipt,
      color: "text-red-700",
    ...
    {
      label: "Honoraires",
      value: `${fmt(data.totalHonoraires)} F CFA`,
      icon: DollarSign,
    ...
    },
    {
      ...
      value: `${fmt(data.totalPaye)} F CFA`,
      icon: CheckCircle,
    ...
    {
      label: "Reste à payer",
      value: `${fmt(data.totalReste)} F CFA`,
      icon: Clock,
